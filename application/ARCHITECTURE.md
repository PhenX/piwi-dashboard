...
| Metadata           | `tags`, `project_tags`, `markers`, `entity_links`, `app_settings`, `test_functions`                                           |
...
| `projects/[id]/test-functions`, `test-functions/[id]`                   | The browser extension's function catalog — CRUD, plus `test-functions/extract` (AI: pasted source → a proposed catalog entry, unsaved draft)                                                                         |
...
| `ai-*.ts`                                                     | Provider abstraction, diagnosis, context building + limits, research stage, embeddings, images, system prompt, function-catalog extraction from pasted code |
...
function catalog — add by hand or via AI extraction from pasted code) · `/test-runs/[id]` · `/test-cases/[id]` (stable
