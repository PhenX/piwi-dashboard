...
- **Per-route required roles**: each route handler declares its allowed roles once as `const REQUIRED_ROLES: Role[] = [...]` (used by `requireAuth`). Nitro's OpenAPI meta extractor can't fold those enum/variable references into the spec, so `app/utils/route-roles.generated.ts` mirrors them into a `{method} {path}` → roles map that the `/docs` UI reads (`routeRoleRequirement`) to show who can call each endpoint. **After changing a route's `REQUIRED_ROLES`, re-run `npm run app:gen:route-roles`** — `tests/unit/route-roles.test.ts` fails if the generated map drifts from the sources. The extraction + file→operation mapping lives in `scripts/route-roles-extract.mjs` (shared by the generator and the test). Roles: `[administrator, reporter, user]` (all three) renders as "Any signed-in user"; a subset excluding `user` is flagged as elevated (admin/reporter-only); routes with no role restriction (public or token-authenticated) are absent from the map.
