...
- **Conventional Commits**: Commits and PR titles MUST follow Conventional Commits (`type(scope): subject`); enforced by a local commitlint hook, a full-history PR lint, and a PR-title lint. See `CONTRIBUTING.md` for the type/scope list and the type→release-bump table — release-please reads PR titles (squash-merge subjects) to compute version bumps and the changelog.
