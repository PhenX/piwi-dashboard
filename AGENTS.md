...
- **Per-route required roles (`x-required-roles`)**: each route's `defineRouteMeta` documents who may call it via `openAPI['x-required-roles']`, and the `/docs` UI reads it from the spec (`routeRoleRequirement`) to render an access line + role chip. **It MUST be a string-literal array** (`'x-required-roles': ['administrator', 'reporter']`) — Nitro's OpenAPI meta extractor only folds inline literals into the spec, so a `REQUIRED_ROLES` variable or `[Role.ADMINISTRATOR]` enum reference is silently dropped/emptied. The values must match the enum values in `Role` (`administrator`/`reporter`/`user`). Enforcement is separate: routes that gate on roles still pass a typed `Role[]` to `requireAuth(event, [...])` (kept in a local `const REQUIRED_ROLES` where used); routes that only require sign-in call `requireAuth(event)` and document `['administrator', 'reporter', 'user']` (rendered as "Any signed-in user"); public/token-authenticated routes omit the field (or use `[]`). A subset excluding `user` renders as elevated (admin/reporter-only). Keep the meta literal in sync with what `requireAuth` actually enforces.
