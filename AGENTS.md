...
- **`reporter/src/internal/`** — private plumbing, grouped by domain:
  ...
  - `capture/` — `capture-fixtures.ts` (Playwright fixtures for network/web-vitals/console/locator capture; runs in the worker), `locator-healing.ts` (pure alternative generation + scoring + `captureCallerLocation`/`suggestLocatorsFromAria`; structurally mirrors `application/shared/locator-healing.types.ts`), `attachments.ts` (the `piwi-*` attachment names)
...
- **`reporter/src/types/`** — `wire.ts` (EXTERNAL server contract; structurally mirrors `shared/types.ts`, never imported from it) and `collected.ts` (INTERNAL in-process model); `reporter/src/types.ts` is a barrel over both
