...
import { buildFailureClues, type FailureClue, type FailureStory, type FailureCluePageDiff } from '../failure-clues';
...
export async function getFailureClues(
  ...
  const [trc] = await db.select().from(testRunsCases).where(eq(testRunsCases.id, id));
  if (!trc) return { clues: [], story: null, failureAt: null };
  ...
  const { clues, story } = buildFailureClues({
    execution: {
      id: trc.id,
      testCaseId: trc.testCaseId,
      status: trc.status,
  ...
  return { clues, story, failureAt: timeline.failureAt };
}
