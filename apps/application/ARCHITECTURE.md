...
| `projects/[id]/selections`, `selections/[key]`                          | Named, data-driven test selections — CRUD, `[key]/resolve` (definition → matching tests + `playwright test` command, what `piwi select` / `piwi run` call) and `preview` (resolve an ad-hoc definition). Resolver + CRUD in `shared/handlers/selections.ts`; the definition shape, validation and materialization in `shared/selection/`          |
