...
<figure>
  <img src="/screenshots/gather-evidence.png" alt="A failing execution: the header, the one-line headline, the clues and the raw error, then one evidence card whose tabs — Timeline, Screen, Source, Network, Console, State, Performance — hold the captured evidence">
  <figcaption>A failing execution — the header, the headline, the strongest clue, the raw error, and one evidence card whose tabs hold everything captured.</figcaption>
</figure>

Below the header the reading order is the same every time: the **headline**, the **clues**, the raw **error** (copyable), then one **evidence** card, then what to do.
...
- **Clues** — the strongest [deterministic clue](#clues) in one line right on the headline (*"GET /api/quote returned 504, 1.1 s before the failure"*), with a **Clues** card below listing every clue, each with a strength chip, its `t-N s` before the failure, and citation chips that switch to the evidence tab it came from and scroll to it.
...
Below the evidence sit **what to do** and the **history**: how the failure clusters across tests, an [AI diagnosis](./ai-diagnosis) of this execution, [alternative locators](./reporter#locator-healing) for a broken locator, and a strip of this test's recent executions linking to its full [test history](#one-execution-diagnosis-first).
