...
**Delivery status (2026-09-05, 17:20 UTC).** Shipped and merged to `main`: all fourteen quick wins, **A** verdict headline (#432), **B** failure timeline with call context (#433), **C** clue engine (#435), **G** loop-closing + partial-run verification (#438), the fix-plan page and diagnosis history (#439), **I** evidence without setup (#440), the quarantine / owner / issue-link actions with bulk triage (#441), the cluster exemplar refresh (#454), **E** attempt diffing (#458), **F** reproduction bundle + generated bisect (#460), **M** the desktop runs the reproduction and drives the bisect (#461), **J** green ARIA samples + structural page diff (#465), plus agent tooling — the `run-app` skill and `app:screens --route` (#434). **L** is being delivered separately as the UI simplification in [`ui-simplification.md`](ui-simplification.md): its execution, cluster, run and project pages, the Home *Open failures* card and every test list are rebuilt as one column with tabbed evidence (PRs #442–#456) and the vocabulary sweep landed (#459) — **L** is complete. In review: **K** resolved-cluster memory (#462). Also in review: **D** the failure inbox (#467). **S** one home for SCM provider hosts and URLs (#466 — a cleanup the maintainer asked for after spotting provider logic re-implemented outside `server/utils/scm/`) was merged into K's branch and reaches `main` with #462. Every item of the plan is now shipped or in review; nothing is left to start. Each list below is marked ✅ shipped / 🔄 in progress / ⬜ not started.
