...
**UI follow-up:** [`ui-simplification.md`](ui-simplification.md) designs big bet **L** (one investigation page with a fixed reading order) for every screen around tests, places the in-progress triage actions, and reserves the spots where **D**, **E**, **F**, **J**, **K**, **N** and **O** land.
...
**Delivery status (2026-09-05, 07:00 UTC).** Shipped and merged to `main`: all fourteen quick wins, **A** verdict headline (#432), **B** failure timeline with call context (#433), **C** clue engine (#435), **G** loop-closing + partial-run verification (#438), the fix-plan page and diagnosis history (#439), **I** evidence without setup (#440), the quarantine / owner / issue-link actions with bulk triage (#441), the cluster exemplar refresh (#454), **E** attempt diffing (#458), **F** reproduction bundle + generated bisect (#460), plus agent tooling — the `run-app` skill and `app:screens --route` (#434). **L** is being delivered separately as the UI simplification in [`ui-simplification.md`](ui-simplification.md): its execution, cluster, run and project pages, the Home *Open failures* card and every test list are rebuilt as one column with tabbed evidence (PRs #442–#456) and the vocabulary sweep landed (#459) — **L** is complete. Not started: big bets **D** (now unblocked), **J**, **K**, **M**, and **N** and **O**, both added 2026-09-05 from the feedback on the r/Playwright launch post. Each list below is marked ✅ shipped / 🔄 in progress / ⬜ not started.
...
- ⬜ **D** the failure inbox. The UI plan's Home *Open failures* card (#442) is D at its smallest; the audit's D is the full inbox on top of it — snooze, assign, bulk actions and a "since you last looked" cut. Ready to start now that the card is in `main`.
...
- ⬜ **M** desktop: *run* the reproduction, don't copy it. F stops at a copy-paste recipe and a `git bisect` script on purpose — they must work for Docker/web users and MCP agents. The desktop shell already spawns the linked folder's own Playwright with the bundled Node (`desktop_run_local_tests`), but it knows nothing about git and nothing about the app under test, so a *Run locally* next to the recipe still runs whatever HEAD the folder is on. M closes that gap, in three parts, all desktop-only and all behind the existing folder link:
  ...
  Constraints that shape the design: never move the user's HEAD (hence the worktree); `npm ci` per step because the lockfile can change inside the window; git required on `PATH`; long runs already raise an OS notification. Builds on F (#460, merged); one session, Rust + Vue + shared + docs.

- ⬜ **N** re-run readback: *reproduced*, *collapsed* or *split*. Added 2026-09-05 from the r/Playwright thread on the launch post, where the most detailed reply asked for exactly this: "show whether the cluster collapses, splits, or reproduces before offering any code change". *Readback* here means the outcome of a re-run, read back into the cluster that asked for it. Today the cluster page can dispatch a CI re-run of every affected test (`server/utils/ci-rerun.ts`, `dispatchClusterRerun`, up to 100 tests) and the desktop shell can run the same tests locally (`app/composables/useDesktopLocalRuns.ts`); the dispatch is stored on the cluster as `lastRerunDispatch` (`shared/ci-rerun.ts`: provider, URL, arguments, time, who). Then nothing. The produced run is ingested like any other, fix verification (`server/utils/fix-verification.ts`, `verifyClusterFixes`) records *stopped-failing* when every affected test passed, and the two other outcomes are never named: a re-run that fails the same way is silent, and one that fails with a different fingerprint moves the tests into another cluster without a word. Four parts:
  1. **Link the dispatch to its run.** Extend `ClusterRerunDispatch` with `runId` and `outcome`. In the run-finalization path, right after `verifyClusterFixes`, match each pending dispatch of the project to the run: the run started after the dispatch and executed every test the dispatch named (a full run qualifies too — the readback is per test). The desktop tray already resolves the run a local run produced (`correlatePiwiRuns`); it records the same fields through the dispatch route with `provider: 'desktop'`.
  2. **Name the outcome, per affected test, from the rows the verifier already loads** (`executedCaseIds`, `passedCaseIds`, the cluster of each failing row): *collapsed* — every test passed (this is *stopped-failing*; the verifier stays the only source of it, the readback only links it to the dispatch); *reproduced* — every test failed again in the same cluster; *split* — at least one test failed in another cluster, new or existing, named in the outcome; *partial* — some passed, some reproduced (*3 of 5 reproduced*), a flakiness signal the leaderboard can count. A dispatched test absent from the run leaves the outcome pending, with a note saying which test did not run.
  3. **Show it where the button is.** One line under *Re-run in CI* on the cluster and execution pages (`app/pages/failure-clusters/[id].vue`, `app/pages/test-run-cases/[id].vue`): *Re-run #123 · reproduced 5 of 5 · 4 min ago*, linking to the run and, on a split, to the clusters it split into; the same line on the *Local runs* tray entry for a desktop run. The person who dispatched gets the line as a targeted notification (`byUserId` is on the dispatch, and `emitNotification` already delivers to one user for the fix author), under a new `cluster.rerun` event. The fix plan and `get_fix_plan` gain a `lastRerun` field; the Markdown export prints the line.
  4. **Re-run one representative.** A second choice beside *Re-run in CI*: dispatch only the test of the cluster's representative execution — the one the cluster page opens, and the one the run page already uses for healing (`shared/handlers/test-runs.ts`, `cases[0]` of each group). One test costs one worker instead of a hundred, and the reply was explicit: "rerun one representative per cluster before assigning owners". Same outcome words, with a count of one.

  Constraints: computed at ingest in the same best-effort block as fix verification (an unreachable SCM or a run that never arrives must never turn into an ingest error); no reporter change; the demo mirror (`app/demo/api/`) seeds one dispatch per outcome so the line is visible without CI. *Split* is the first measured signal of over-grouping: count splits per project so **K** and the fingerprint's masking rules have a number to work against. Builds on **G** (#438) and the CI re-run; one session, server + shared + Vue + demo + docs.

- ⬜ **O** run-level cause grouping: the earliest shared divergence. Added 2026-09-05 from the same thread: "for 40 red tests, i'd cluster on the earliest shared divergence rather than the final error: the same fixture, dependency or request, console exception, changed file, worker, or environment". A *cause group* is a set of failing executions of one run that share one such signal. Clusters key on the final error's fingerprint (`shared/error-fingerprint.ts`), so a dependency that answers 503 on `/api/quote` becomes a timeout in one spec, a `toBeEnabled` assertion in a second and a strict-mode error in a third — three clusters, one cause, three diagnoses. The clue engine (**C**, `shared/failure-clues.ts`) already detects every signal the reply lists, per execution: `slow-request-overlapping-failure`, `console-mentions-target`, `backend-error-attached`, `worker-pollution`, `wrong-page`, `environment-changed`, `browser-specific`. It only ever runs for one execution at a time (`getFailureClues` in `shared/handlers/test-cases.ts`) and its results are never compared across a run. Four parts:
  1. **A cause key per failing execution.** In the run-finalization path, after clusters are assigned and in the same fire-and-forget block as the embedding reconciler, take the run's failing executions (capped at 200, largest clusters first) and derive from the cheap clue inputs — network rows, console entries, worker order, the app-state URL, the error text — one key per signal that names something *shared*, volatile parts masked with the fingerprint's `maskVolatile`: `request:GET /api/quote:5xx`, `console:<masked line>`, `backend:<masked message>`, `worker:<index>:<predecessor test>`, `page:/login`, `hook:<file>:beforeAll` (Playwright repeats the hook error in every test of the file, so the error text is enough). Healing and the environment diff are skipped here: both resolve a per-execution baseline and are too costly for a run-wide pass; an environment key counts only when it does *not* cover every failure of the run.
  2. **Group and rank.** Executions sharing a key form a cause group; a group is kept when it spans at least two clusters or half the run's failures, ranked by failures covered, strongest signal first. Stored once per run as JSON (`test_runs.cause_groups`: key, kind, label, cluster ids, execution ids, strength) and computed on demand for runs ingested before the column existed. A pair of clusters that share a key in two consecutive runs records a `cluster_merge_suggestions` row with `method: 'shared-clue'` (`shared/handlers/cluster-merge-suggestions.ts`), reusing the approve / reject flow — a suggestion, never a merge.
  3. **Show it as the first line about the run.** Above the run page's cluster list (`app/pages/test-runs/[id].vue`): *27 of 40 failures share one cause — GET /api/quote answered 503 just before each failure*, expanding to the clusters and tests it covers. On each cluster in a group, one line in the header: *Likely shares a cause with clusters #12 and #14*. The run alert and the PR comment (`server/utils/scm/pr-feedback.ts`) lead with the same sentence when one group covers most of the failures. The reply's own test: "if one fix collapses 27 failures, that's a cause".
  4. **Feed N and the model.** *Re-run one representative* (**N**) accepts a cause group as well as a cluster — one test per group, the cheapest possible check of the grouping. The cause group becomes a section of the diagnosis prompt for every cluster it contains (`server/utils/ai-context.ts`, next to the clues section), so the model diagnoses the cause once instead of once per cluster.

  Constraints: deterministic, no model call; the same pure function runs in the demo; clusters are never merged by this pass; the *changed file* signal from the reply is left out on purpose — mapping the regression window's diff to the tests it broke needs source knowledge that only the diagnosis context has today, and belongs with **K**. Builds on **C** (#435); one session, server + shared + Vue + demo + docs.
