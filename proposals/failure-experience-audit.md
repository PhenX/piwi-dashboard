...
**Delivery status (2026-09-05).** Shipped and merged to `main`: all fourteen quick wins, **A** verdict headline (#432), **B** failure timeline with call context (#433), **C** clue engine (#435), **G** loop-closing + partial-run verification (#438), the fix-plan page and diagnosis history (#439), **I** evidence without setup (#440), the quarantine / owner / issue-link actions with bulk triage (#441), the cluster exemplar refresh (#454), **E** attempt diffing (#458), plus agent tooling — the `run-app` skill and `app:screens --route` (#434). In review: **F** reproduction bundle + generated bisect (#460, CI green). **L** is being delivered separately as the UI simplification in [`ui-simplification.md`](ui-simplification.md): the execution, cluster (#453), run (#450) and project (#451) pages are rebuilt as one column with tabbed evidence and every list renders the shared `TestRow` (#456); the vocabulary sweep is in review (#459) and the Home *Open failures* card is still to land. Not started: big bets **D**, **J**, **K** and the new **M**. Each list below is marked ✅ shipped / 🔄 in progress / ⬜ not started.
...
- 🔄 **L** the unified investigation layout — the UI simplification. Execution, cluster (#453), run (#450) and project (#451) pages rebuilt and every list on the shared `TestRow` (#456), all in `main`; the vocabulary/docs sweep is in review (#459) and the Home *Open failures* card remains. Tracked in [`ui-simplification.md`](ui-simplification.md), not restarted here.
