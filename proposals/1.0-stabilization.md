...
call applies). Six batches shipped (see Settled): the lowest-fanout bare arrays, the content CRUD family, the
settings/user CRUD, the project analytics (`performance` with `limit`→`runs`, `slow-tests`), `flaky-tests` + project
`failure-clusters`, and the `projects/menu`/`projects/overview`/`test-runs/recent` listing routes. What remains is
`GET /api/projects` — asserted as a bare array across ~10 E2E files, the single highest-fanout route — plus aligning
the catalog and the `commits` composite. Both land as their own commits to keep the app green.
