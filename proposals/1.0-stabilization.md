...
call applies). Two batches shipped (see Settled): the lowest-fanout bare arrays, and the content CRUD family
(`tags`, `links`, `markers`, `test-functions`, `branches`, the three `share-links`). What remains is the high-fanout
bare-array routes read across the E2E suite (`projects`, `projects/menu`, `projects/overview`, `test-runs/recent`,
`flaky-tests`, `slow-tests`, `performance`, project `failure-clusters`) and the settings/user CRUD routes
(`channels`, `subscriptions`, `members`, `users`, `api-keys`), plus aligning the catalog and the `commits` composite
and renaming `performance`'s `limit`→`runs`. Each endpoint is a server + UI + demo + E2E change, so the bulk lands
endpoint by endpoint to keep the app green.
