...
call applies). Four batches shipped (see Settled): the lowest-fanout bare arrays, the content CRUD family, the
settings/user CRUD, and the project analytics `performance` (with `limit`→`runs`) and `slow-tests`. What remains is
the highest-fanout bare-array routes read across the E2E suite (`projects`, `projects/menu`, `projects/overview`,
`test-runs/recent`, `flaky-tests`, project `failure-clusters`), plus aligning the catalog and the `commits`
composite. Each endpoint is a server + UI + demo + E2E change, so the bulk lands endpoint by endpoint to keep the app
green.
...
- **List envelopes, project analytics (part of D6)** — project `performance` and `slow-tests` now return `{ items }`,
  ...
  bare arrays (`projects`, `flaky-tests`, …) plus the catalog/`commits` finish-up stay open under D6.
