...
The OpenAPI-accuracy items shipped (see Settled). What remains: quarantine release passes mutation metadata as a
query param on DELETE, unlike every other mutation (the one behavior-changing item — move it into the request body);
and a handful of routes are tagged into the wrong `/docs` group.
...
- **Route OpenAPI accuracy (part of D16)** — the query parameters routes actually read are now declared in their
  ...
  marked experimental so its shape is not frozen. The quarantine-DELETE body move and the doc-group retagging stay
  open under D16.
