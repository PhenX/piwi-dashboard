...
**Decided:** every list route standardizes on a `{ items: [...] }` envelope; `limit`/`offset` are the row-pagination
vocabulary while `runs`/`days`/`buckets` are documented domain windows (no aliases — the "no backward compatibility"
call applies). Every bare-array and `{ <noun> }` list route is now enveloped (seven batches, see Settled) — including
`GET /api/projects`, the highest-fanout route. What remains is only the finish-up: the test-case catalog already
carries `{ items, total, limit, offset }` (keep), and the cluster `commits` composite (`{ commits, repositoryUrl,
aggregate, error, hasMore }`) still names its collection `commits` — rename to `items` beside its metadata to close
D6.
...
The OpenAPI-accuracy items and the quarantine-DELETE body move shipped (see Settled). What remains is the fuzziest
item: a handful of routes may be tagged into the wrong `/docs` group. The tag taxonomy is by subject entity, not URL
prefix, and the flagged cases (e.g. project `performance` tagged `Test Runs`) are defensible rather than clearly
wrong — left open pending a call on the intended grouping.
...
- **List envelopes, first batch (part of D6)** — the seven lowest-fanout bare-array list routes now return the
  standard `{ items: [...] }` envelope so they can grow metadata post-1.0 without a break: execution `traces`,
  test-case `history`, run `failure-groups`, run `network-requests`, project `timeout-opportunities`, project
  `cluster-merge-suggestions`, and cluster `diagnoses`. Server routes, the demo router, the UI consumers (via a
  `transform` that keeps downstream code array-shaped), and the E2E specs all move together.
- **List envelopes, content CRUD (part of D6)** — the content list routes that used `{ <noun>: [...] }` now return
  `{ items: [...] }`: `tags`, `links`, project `markers`, project `test-functions`, cluster `branches`, and the three
  entity `share-links` (project/cluster/execution). The shared handlers and the MCP tools that call them keep their
  `{ <noun> }` shape (MCP is a separate surface); only the REST routes and the demo router rewrap. The named response
  types (`TagsResponse`/`MarkersResponse`/`TestFunctionsResponse`) renamed their field to `items`, so TypeScript
  flagged every consumer. The high-fanout bare arrays and the settings/user CRUD routes stay open under D6.
- **List envelopes, settings/user CRUD (part of D6)** — `channels`, `subscriptions`, project `members`, `users`
  (composite `{ items, authEnabled }`), and user `api-keys` now return `{ items }`. `ProjectMembersResponse`,
  `UsersResponse` and `ApiKeysResponse` renamed their field to `items` (TypeScript flagged the consumers); the inline
  channel/subscription types in the notification UI and the notifications/user-management E2E specs moved with them.
  Only the high-fanout bare arrays (plus the catalog/`commits` finish-up) stay open under D6.
- **List envelopes, project analytics (part of D6)** — project `performance` and `slow-tests` now return `{ items }`,
  and `performance`'s recent-runs window param is renamed `limit`→`runs` so the same concept has one name across the
  analytics routes (the UI never sent it; `limit`/`offset` are now reserved for row pagination). Server routes, demo
  router, the project-detail UI fetches, and the performance-api E2E spec move together. The remaining highest-fanout
  bare arrays (`projects`, …) plus the catalog/`commits` finish-up stay open under D6.
- **List envelopes, flaky-tests + clusters (part of D6)** — project `flaky-tests` and project `failure-clusters` now
  return `{ items }`. Their list components use a `transform` to stay array-shaped; the flaky-tests, failure-clusters,
  fix-verification, fix-plan and ai-diagnosis E2E specs read `.items`. Remaining under D6: `GET /api/projects` and the
  catalog/`commits` finish-up.
- **List envelopes, project/run listings (part of D6)** — `projects/menu`, `projects/overview` and `test-runs/recent`
  now return `{ items }`. The home/analytics/sidebar/desktop-import consumers use a `transform`; the demo router
  rewraps; the dashboard-endpoints, import-runs, failure-clusters and case-files-live E2E specs read `.items`. Only
  `GET /api/projects` (highest fanout) and the catalog/`commits` finish-up stay open under D6.
- **List envelopes, `GET /api/projects` (part of D6)** — the single highest-fanout list route now returns `{ items }`.
  The layout sidebar and projects page use a `transform` (shared `key: 'projects'`, same transform); the demo router
  rewraps; and ~24 call-sites across a dozen E2E specs (api-server, environment, postgresql, reporter-with-auth,
  fix-verification, fix-plan, projects-create, tags, keyboard-nav, cluster-page-layout, mobile-responsiveness,
  quarantine) read `.items`. Only the catalog/`commits` finish-up stays open under D6.
- **Route OpenAPI accuracy (part of D16)** — the query parameters routes actually read are now declared in their
  OpenAPI: `force` on the three diagnosis POSTs (`test-run-cases/[id]/diagnose`, `failure-clusters/[id]/diagnose`
  and its `/stream`), `limit`/`from`/`to`/`fullRunsOnly` on project `performance`, `runs` on `slow-tests`,
  ...
  caller). Only the doc-group retagging stays open under D16.
