...
call applies). Five batches shipped (see Settled): the lowest-fanout bare arrays, the content CRUD family, the
settings/user CRUD, the project analytics (`performance` with `limit`→`runs`, `slow-tests`), and `flaky-tests` +
project `failure-clusters`. What remains is the project/run listing routes (`projects`, `projects/menu`,
`projects/overview`, `test-runs/recent`) — `projects` alone is asserted as a bare array across ~10 E2E files — plus
aligning the catalog and the `commits` composite. Each endpoint is a server + UI + demo + E2E change, so the bulk
lands endpoint by endpoint to keep the app green.
...
- **List envelopes, flaky-tests + clusters (part of D6)** — project `flaky-tests` and project `failure-clusters` now
  ...
  fix-verification, fix-plan and ai-diagnosis E2E specs read `.items`. Remaining under D6: the project/run listing
  routes (`projects`, `projects/menu`, `projects/overview`, `test-runs/recent`) and the catalog/`commits` finish-up.
