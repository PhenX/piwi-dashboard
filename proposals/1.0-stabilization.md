...
call applies). The first, lowest-fanout batch shipped (see Settled). What remains is the high-fanout bulk: the
bare-array routes read across the E2E suite (`projects`, `projects/menu`, `projects/overview`, `test-runs/recent`,
`flaky-tests`, `slow-tests`, `performance`, project `failure-clusters`) and the `{ <noun>: [...] }` CRUD family
(`tags`, `links`, `channels`, `members`, `branches`, `markers`, `test-functions`, `subscriptions`, `users`,
`api-keys`, the three `share-links`), plus aligning the catalog and the `commits` composite and renaming
`performance`'s `limit`→`runs`. Each endpoint is a server + UI + demo + E2E change, so the bulk lands endpoint by
endpoint to keep the app green.
...
- **List envelopes, first batch (part of D6)** — the seven lowest-fanout bare-array list routes now return the
  ...
  `transform` that keeps downstream code array-shaped), and the E2E specs all move together. The high-fanout routes
  and the CRUD `{ <noun> }` family stay open under D6.
