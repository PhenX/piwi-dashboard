...
call applies). Three batches shipped (see Settled): the lowest-fanout bare arrays, the content CRUD family, and the
settings/user CRUD (`channels`, `subscriptions`, `members`, `users`, `api-keys`). What remains is the high-fanout
bare-array routes read across the E2E suite (`projects`, `projects/menu`, `projects/overview`, `test-runs/recent`,
`flaky-tests`, `slow-tests`, `performance`, project `failure-clusters`), plus aligning the catalog and the `commits`
composite and renaming `performance`'s `limit`→`runs`. Each endpoint is a server + UI + demo + E2E change, so the
bulk lands endpoint by endpoint to keep the app green.
...
- **List envelopes, settings/user CRUD (part of D6)** — `channels`, `subscriptions`, project `members`, `users`
  ...
  Only the high-fanout bare arrays (plus the catalog/`commits`/`performance` finish-up) stay open under D6.
