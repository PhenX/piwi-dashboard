...
- The peer range `^1.61.1` already admits 1.63; nothing here needs a floor bump. Every read is feature-detected (`'subtitle' in step`, `Array.isArray(test._locks)`, trace events by `type`), as `ariaSnapshotBestEffort` does today, so a 1.61 project loses nothing and a 1.63 project gains without configuration.
